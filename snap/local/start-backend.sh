...
BACKEND_USER="$(snapctl get backend.user)"
BACKEND_USER="${BACKEND_USER:-ubuntu}"

cd "$SNAP"
exec su -s /bin/sh "$BACKEND_USER" -c "PYTHONHOME='$PYTHONHOME' PYTHONPATH='$PYTHONPATH' PATH='$PATH' exec '$SNAP/bin/python3' '$SNAP/bin/flask' --app app run --host 0.0.0.0 --port 5000"
