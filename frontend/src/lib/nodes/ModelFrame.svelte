...
<div class="model-frame rounded-xl p-4 w-full h-full min-w-[400px] min-h-[300px] transition-all duration-200 {frameClasses} {borderStyle}">
  <!-- Frame Header -->
  <div class="flex items-center justify-between mb-3 pb-2 border-b nopan {isDropTarget ? 'border-green-400' : 'border-blue-300'}">
    <div>
      <h2 class="text-xl font-bold {isDropTarget ? 'text-green-800' : 'text-blue-800'}">
        Model: {modelName}
      </h2>
      {#if isDropTarget}
    ...
    <div class="relative nopan">
      ...
      {#if showDropdown}
        <div class="absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-lg border border-gray-200 p-4 z-10">
          <div class="mb-3">
          ...
          <button
            ...
            disabled={!autoRefreshInterval || parseInt(autoRefreshInterval) <= 0}
            class="w-full px-4 py-2 text-sm font-medium text-white rounded-md transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed {isAutoRefreshEnabled ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}"
          >
  ...
  <div class="frame-content min-h-[200px] relative">
    {#if modelStatus}
      <div class="absolute top-2 right-2 bg-white rounded-lg shadow p-2 text-xs max-w-[200px]">
    ...
    {:else if isDropTarget}
      <div class="text-center text-green-600 text-lg mt-16 font-bold flex items-center justify-center animate-pulse">
